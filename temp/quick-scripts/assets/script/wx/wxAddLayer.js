(function() {"use strict";var __module = CC_EDITOR ? module : {exports:{}};var __filename = 'preview-scripts/assets/script/wx/wxAddLayer.js';var __require = CC_EDITOR ? function (request) {return cc.require(request, require);} : function (request) {return cc.require(request, __filename);};function __define (exports, require, module) {"use strict";
cc._RF.push(module, 'c24ecqoj5BLtKnPVBDlxLG2', 'wxAddLayer', __filename);
// script/wx/wxAddLayer.js
...
cc.Class({
     ...
     * @param {*无意义} floats 
     */
    callback: function callback(floats) {
     ...
     * 进入其他界面，重新更新 金币
     */
    setShareAssist: function setShareAssist(callback) {
     ...
     * @param {*} callback 
     */
    showHelpTips: function showHelpTips(callback) {
    ...
    showMeReq: function showMeReq(callBack) {
    ...
    assistTime: function assistTime() {
        if (self._asTimes <= 0) {
            self.showMeRequst();
    ...
    showMeRequst: function showMeRequst() {
     ...
     * 
     */
    rankBtn: function rankBtn(event, customData) {
    ...
    rankQunBtn: function rankQunBtn(event, customData) {
     ...
     * gold 分享获得金币 数量，
     */
    shareBtnWithScore: function shareBtnWithScore(event, callBack, gold) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     * 友助 复活
     */
    shareBtnWithRevived: function shareBtnWithRevived(event, callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            var func = function func() {
                self.assistLayerTwo.active = true;
                var js = self.assistLayerTwo.getComponent("assistLayerJs");
                js.init(callBack);
            };
     ...
     * 
     */
    shareBtnWithAssit: function shareBtnWithAssit(event, callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     * 
     */
    shareBtn: function shareBtn(event, callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            self._localScore = self._userOperate.getLocalScore();
            if (self._localScore) {
     ...
     * 
     */
    shareBtnWithUrl: function shareBtnWithUrl(event, callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     */
    shareBtnWithGroup: function shareBtnWithGroup(event, callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     * @param {*} callBack 
     */
    shareBtnWithSpecial: function shareBtnWithSpecial(event, callBack, obj) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     * 获取分享次数
     */
    getShareTicketTime: function getShareTicketTime() {
     ...
     * 红包
     */
    hongbaoBtn: function hongbaoBtn() {
        //红包功能未完成 需要开发后调试
        // if(cc.sys.platform == cc.sys.WECHAT_GAME){
        //     self.closeTouch()
     ...
     * 
     */
    getFriendsRankBtn: function getFriendsRankBtn() {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     * 
     */
    setNextFriendInfoBtn: function setNextFriendInfoBtn() {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
    ...
    * 
    */
    getOpenDataQunRankBtn: function getOpenDataQunRankBtn() {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     * //直接拉到控制按键就好
     */
    assistBtn: function assistBtn(event) {
        self.myAssistWidget.active = true;
        var js = self.myAssistWidget.getComponent("myAssistLayer");
        js.init(event);
    },
     ...
     *    金币复活，分享互助 2合一接口
     */
    assistTwoBtn: function assistTwoBtn(callback, gold) {
    ...
    * 飘字
    */
    _flyText: function _flyText(_text, color, strokeColor, time) {
        var textNode = new cc.Node();
        textNode.addComponent(cc.Label);
        textNode.addComponent(cc.LabelOutline);
        var winSize = self.node.getContentSize();
    ...
    */
     ...
     */
    checkScore: function checkScore(score) {
     ...
     * @param {*} score 
     */
    setCurLocalScore: function setCurLocalScore(score) {
        if (typeof score == 'number') {
    ...
    *体力操作
    */
    getVit: function getVit() {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            global.setUserDataInfoByName('vit', res.vit);
        }
        return 0;
    },
    subVit: function subVit(callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
            self._userOperate.getVit(function (res) {
                global.setUserDataInfoByName('vit', res.vit);
                if (parseInt(res.vit) > 0) {
     ...
     * 设置用户自定义数据上传
     */
    setCustomData: function setCustomData(blob, callBack) {
        if (cc.sys.platform == cc.sys.WECHAT_GAME) {
     ...
     */
    subGold: function subGold(_num, callBack) {
        consume.subGold(_num, callBack);
    },
    addGold: function addGold(_num) {
    ...
    *打开 体力定时器 30分钟恢复一次体力；
    */
    openVitTimers: function openVitTimers() {
     ...
     * 开奖操作
     */
    startActivity: function startActivity(callBack) {
        lottery.startActivity(callBack);
    },
    runLottery: function runLottery(type, callBack) {
        lottery.runLottery(type, callBack);
    },
    quitActivity: function quitActivity(callBack) {
        lottery.quitActivity(callBack);
    },


    //保存公众号;
    saveOfficalAccount: function saveOfficalAccount() {
    ...
    //显隐公众号
    officalAccActive: function officalAccActive(show) {
     ...
     * 添加 游戏开始次数 
     */
    addGameStartTimes: function addGameStartTimes() {
     ...
     * 获得 游戏开始次数
     */
    getGameStartTimes: function getGameStartTimes() {
...
cc._RF.pop();
        ...
        //# sourceMappingURL=wxAddLayer.js.map
